...
const FILTER_DUE_DAYS = 90;          // notify after 90 days
...
export async function runNotificationChecks(userId) {
  ...
  try {
    ...
    // ── D. Filter due for replacement ─────────────────────────────────────
    if (rfidLogs && rfidLogs.length > 0) {
      // Find when current filter was first detected
      const currentRfid = rfidLogs[0].rfid;
      const allWithCurrent = rfidLogs.filter(l => l.rfid === currentRfid);
      ...
      if (firstSeen) {
        ...
        if (daysSince >= FILTER_DUE_DAYS) {
          const urgency = daysSince >= 180
          ...
          await sendNotification(
            "filter_due",
            `${urgency.emoji} Filter Replacement Due`,
            `Your HVAC filter has been installed for ${daysSince} days. Consider replacing it ${urgency.level}.`,
            { type: "filter_due", daysSince }
          );
        }
      }
    }
